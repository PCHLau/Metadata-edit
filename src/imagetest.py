...
def cropper(url: str) -> tuple[int, int]:
    ...
    if im.size == (1280, 720):
        ...
        if (
            np.max(r) - np.min(r) < 10
            and np.max(g) - np.min(g) < 10
            and np.max(b) - np.min(b) < 10
        ):
            box = (280, 0, 1000, 720)
            region = im.crop(box)
            im_size = region.size
    ...
    # might need to add functionality later for determining if image needs to be squared at all
        ...
        try:
            ...
            for i in range(im.size[1]):
                ...
                rgby.append(y)
                if len(rgbx) > 2:
                    # check to see if successive pixels are of the same colour
                    for j in range(3):
                        if (
                            abs(rgbx[-1][j] - rgbx[-2][j]) > 10
                            or abs(rgby[-1][j] - rgby[-2][j]) > 10
                        ):
                            broke = True
                            break
                diff = i
